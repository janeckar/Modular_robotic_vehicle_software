...
//
// Original file: pid_controller.cpp

/***********************************
 * modified by: Radek Janečka      *
 * email: gitjaneckar@seznam.cz    *
 * date: 3. 4. 2025                *
 * file: pwm_pid_controller.cpp    *
 ***********************************/

#include "pwm_pid_controller/pwm_pid_controller.hpp"
...
using ControllerCommandMsg = pwm_pid_controller::PwmPidController::ControllerReferenceMsg;
...
controller_interface::CallbackReturn PwmPidController::on_init(){
  try
  {
...
void PwmPidController::update_parameters()
{
  if (param_listener_->is_old(params_))
  {
    ...
    for(size_t i = 0; i < pids_.size(); i++){
      // if the range parameters are incorrect the changes won't be reflected
      pids_[i]->set_gains_from_parameter(params_.gains.dof_names_map.at(params_.dof_names[i]));
    }
  }
}

controller_interface::CallbackReturn PwmPidController::configure_parameters(){
  update_parameters();
  ...
  for (size_t i = 0; i < dof_; ++i)
  {
    ...
    pids_[i] =
      std::make_shared<pid_regulator>(); // get_node(), "gains." + params_.dof_names[i], true);
    if (!pids_[i]->set_gains_from_parameter(params_.gains.dof_names_map.at(params_.dof_names[i])))
    {
      RCLCPP_FATAL(
        get_node()->get_logger(),
        "Setting PID parameters for joint '%s' was unsuccessful, probably the range with min max values is reversed.",
        params_.dof_names[i].c_str());
      return CallbackReturn::FAILURE;
    }
  }
...
controller_interface::CallbackReturn PwmPidController::on_cleanup(
...
controller_interface::CallbackReturn PwmPidController::on_configure(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  ref_subscriber_ = get_node()->create_subscription<ControllerReferenceMsg>(
    ...
    std::bind(&PwmPidController::reference_callback, this, std::placeholders::_1));
...
void PwmPidController::reference_callback(const std::shared_ptr<ControllerReferenceMsg> msg)
{
...
controller_interface::InterfaceConfiguration PwmPidController::command_interface_configuration() const
{
...
controller_interface::InterfaceConfiguration PwmPidController::state_interface_configuration() const
{
...
std::vector<hardware_interface::CommandInterface> PwmPidController::on_export_reference_interfaces()
{
...
std::vector<hardware_interface::StateInterface> PwmPidController::on_export_state_interfaces()
{
...
bool PwmPidController::on_set_chained_mode(bool /*chained_mode*/)
{
  // Always accept switch to/from chained mode
  return true;
}

controller_interface::CallbackReturn PwmPidController::on_activate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  // resets internal state of pid regulators back to initial state
  for (auto & pid : pids_)
  {
    pid->reset();
  }
...
controller_interface::return_type PwmPidController::update_reference_from_subscribers(
...
controller_interface::return_type PwmPidController::update_and_write_commands(
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  for (size_t i = 0; i < dof_; ++i)
  {
    ...
    if (std::isfinite(reference_interfaces_[i]) && std::isfinite(measured_state_values_[i]))
    {
      ...
      {
        RCLCPP_ERROR(get_node()->get_logger(), "Two state interfaces are available but using only the first!");
      }
  
      // use calculation with 'error' only
      tmp_command = pids_[i]->compute_command_dead_zone(reference_interfaces_[i], measured_state_values_[i], period);
...
PLUGINLIB_EXPORT_CLASS(
  pwm_pid_controller::PwmPidController, controller_interface::ChainableControllerInterface)
